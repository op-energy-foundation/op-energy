...
module OpEnergy.BlockTimeStrike.Server.V1.DB.Migrations.CalculateBlockTimeStrikeGuessesCountSlowFast.BlockTimeStrikeGuessAfter
  where
...
share [mkPersist sqlSettings, mkMigrate "createCalculatedBlockTimeStrikeGuessesCountSlowFastAfter"] [persistLowerCase|
...
CalculatedBlockTimeStrikeGuessesCount
  ...
  fastCount (Natural Int) -- calculations are done, now we are ready to drop Maybe here
  slowCount (Natural Int) -- calculations are done, now we are ready to drop Maybe here
  UniqueCalculatedBlockTimeStrikeGuessesCountStrike strike
  deriving Eq Show Generic
|]
