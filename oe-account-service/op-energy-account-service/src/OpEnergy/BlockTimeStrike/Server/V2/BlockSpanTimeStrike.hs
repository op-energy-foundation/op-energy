...
apiBlockSpanTimeStrikeModelBlockTimeStrike
  ...
  -> AppT m (Either CallstackError API.BlockSpanTimeStrike)
  ...
  mBlockSpan <- do
    if APIV1.blockTimeStrikeBlock v > APIV1.blockHeaderHeight confirmedBlock
      then return Nothing
        ...
        eBlockSpan <- withExceptT (blockspanAPIError . tshow) $ ExceptT
          $ liftIO $ eitherException
            $ BlockSpanClient.withClientEither url $ do
              BlockSpanClient.v2getSingleBlockspan
                ...
                (Just spanSize)
        eret <- withExceptT (blockspanAPIError . tshow) $ ExceptT
          $ return eBlockSpan
        return $! Just eret
