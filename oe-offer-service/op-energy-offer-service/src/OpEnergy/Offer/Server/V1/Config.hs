...
data Config = Config
  ...
  , configSchedulerPollRateSecs :: Positive Int
    -- ^ scheduler interval -- drives OpEnergy.Offer.Server.V2.Expiry's sweep
  ...
  , configAccountServiceURL :: BaseUrl
    -- ^ base URL of oe-account-service's HTTP API, used to resolve an
    -- AccountToken to its owning account's identity (personUUID/displayName)
    -- via GET /api/v2/account/whoami, and to deduct/credit that account's
    -- balance via internal/balance/{deduct,credit} -- see
    -- OpEnergy.Offer.Server.V1.AccountClient. This service keeps no balance
    -- of its own: oe-account-service's Person table is the only copy.
  , configInternalServiceSharedSecret :: Text
    -- ^ sent as the X-Internal-Service-Secret header on every
    -- internal/balance/{deduct,credit} call -- must match
    -- oe-account-service's own configInternalServiceSharedSecret exactly.
  }
  deriving Show
...
defaultConfig = Config
  ...
  , configInternalServiceSharedSecret = error "defaultConfig: you are missing INTERNAL_SERVICE_SHARED_SECRET from config -- must match oe-account-service's own value. Generate one with \"dd if=/dev/urandom bs=1 count=32 2>/dev/null | base64 -w 0\" command"
  }
