...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    nixpkgs.overlays = [
      op-energy-overlay # add op-energy-offer-service into context
    ];
    environment.systemPackages = [ pkgs.op-energy-api pkgs.op-energy-offer-service ];
    # enable postgresql and declare op-energy offer DB
    services.postgresql = {
      enable = true;
        ...
        preStart = ''
          ...
          if [ ! "$(sudo -u postgres psql -l -x --csv | grep 'Name,${cfg.db_name}' --count)" == "1" ]; then
            ( echo 'CREATE DATABASE ${cfg.db_name};'
              echo '\c ${cfg.db_name};'
            ) | sudo -u postgres psql || true # other op-energy services can be running in parallel
          fi
          cat "${initial_script cfg}" | sudo -u postgres psql || true # other op-energy services can be running in parallel
        '';
        script = "exit 0";
      };
      op-energy-offer-service =
      let
        openergy_config = pkgs.writeText "op-energy-offer-service-config.json" cfg.config; # this renders config and stores in /nix/store
      in {
        wantedBy = [ "multi-user.target" ];
        after = [
